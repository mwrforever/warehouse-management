...
- 从订单生成的明细行数量输入允许 0（`el-input-number min=0`），0 行显示灰字「本次不收货」。
- 提交前 `filter(qty > 0)`；若过滤后为空 → 提示「请至少录入一个收货数量大于 0 的商品」。
- **上限校验说明（实现差异，2026-08-22 评审标注）**：超额部分不做过量前端校验——编辑草稿场景详情接口不携带剩余量数据，无法统一实现 blur 上限回弹；超剩余量由后端草稿期 `validateOrderItems`（1308）拦截（与既有行为一致）。负数量前端直接拦截「数量不能小于 0」（与后端 1302 同口径）。
