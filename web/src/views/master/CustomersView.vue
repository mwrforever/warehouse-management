  ...
  <div class="page-card">
    <div class="toolbar">
      ...
      <div class="toolbar-right">
        ...
        <el-button v-if="auth.has('customer.create')" class="btn-primary" @click="openCreate">新 建</el-button>
      </div>
    </div>
    <el-table :data="rows" v-loading="loading">
      ...
      <el-table-column label="操作" width="160" fixed="right">
        <template #default="{ row }">
          <el-button v-if="auth.has('customer.update')" link type="primary" @click="openEdit(row)">编 辑</el-button>
          <el-button v-if="auth.has('customer.delete')" link type="danger" @click="remove(row)">删 除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
async function save() {
  ...
  saving.value = true
  try {
    if (form.id) await customerApi.update(form.id, { name: form.name, code: form.code, contact: form.contact, phone: form.phone, address: form.address, remark: form.remark, status: form.status })
    else await customerApi.create({ name: form.name, code: form.code, contact: form.contact, phone: form.phone, address: form.address, remark: form.remark, status: form.status })
