...
- **库位错配修正（本计划 Task 4 测试实测发现）**：种子 SEMI-001 仅 @A-01，而 TC-SAL-02 的 from-order 出库单整单落 B-01（含 SEMI-001 行）→ 审核必 1409。TC-SAL-01 在记录基线后用采购入库 API 给 SEMI-001@B-01 补 50 库存（供应商 SUP-001 幂等解析），此后 TC-SAL-02/05 的扣减发生在 B-01（S₂b 基线），A-01 的 S₂ 仅被 TC-SAL-08 触碰
...
test.describe('销售管理模块', () => {
  ...
  test('TC-SAL-01 销售订单创建与审核（原料禁售）', async ({ page }) => {
    ...
    if (supList.total > 0) {
      ...
      const supAfter = await apiGet(page, '/api/v1/suppliers', { keyword: 'SUP-001', per_page: 100 })
    ...
    const prepApprove = await apiPost(page, `/api/v1/purchase/inbounds/${prepPiList.items[0].id}/approve`)
    ...
    const s2b = Number(b01Row.quantity)
    expect(s2b).toBe(50)
  ...
  test('TC-SAL-08 独立出库与边界（余额 0/重复商品）', async ({ page }) => {
    ...
    const semiA01 = semi.items.find(
      (r: { location_id: number }) => r.location_id === a018.id,
    ) as { quantity: number; product_id: number; warehouse_id: number; location_id: number }
...
- §4「失败处理说明」追加：**TC-SAL-05 数据流调整记录**——from-order 弹窗无删除明细行能力（产品决策点，与采购 TC-PUR-05 同源），首张出库单必然含全部剩余行（TC-SAL-02 含 SEMI-001×5 全量），TC-SAL-05 预填仅 FIN-002 剩余 4 一行；文档原「FIN-002 剩余 4 + SEMI-001 全部 5」按实际可达流程修正。**库位错配修正记录**——种子 SEMI-001 仅 @A-01 而订单出库单整单落 B-01，TC-SAL-01 用采购入库 API 补 SEMI-001@B-01=50（S₂b 基线），TC-SAL-02/05 扣减在 B-01、TC-SAL-08 独立出库走 A-01
