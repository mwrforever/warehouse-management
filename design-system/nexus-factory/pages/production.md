...
- 弹窗 900px；次级弹窗豁免 900px 规则：缺料警告（560px）/委外回收（560px）/回收记录（700px）等保持小尺寸
- 本模块 V1 无扫码入口（无扫码框，不实现「扫码自动聚焦（命中清空/未命中保留）」）
- 状态标签（el-tag）：
...
- 操作分流：草稿→「编 辑/删 除/下 达」；已下达→「开 工/详 情」；生产中→「领 料/退 料/报 工/委 外/成品入库/完 工/详 情」；已完成→「关 闭/详 情」
...
- 数量输入 el-input-number precision=2；弹窗关闭二次确认（同销售未实施项标注）
