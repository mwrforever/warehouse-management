...
// 编号规则配置控制器：列表/预览 读取 + 编辑薄壳（写流程下沉 DocumentNumberConfigService；
// Spec 2 配置驱动；类型由种子固定，无新建/删除）

namespace App\Http\Controllers\Api;

use App\Http\Controllers\Controller;
use App\Http\Requests\Master\PreviewDocumentNumberConfigRequest;
use App\Http\Requests\Master\SaveDocumentNumberConfigRequest;
use App\Models\DocumentNumberConfig;
use App\Services\DocumentNumberConfigService;
...
class DocumentNumberConfigController extends Controller
{
    ...
    public function __construct(private DocumentNumberConfigService $documentNumberConfigService) {}
    ...
    public function update(SaveDocumentNumberConfigRequest $request, DocumentNumberConfig $config)
    {
        // 写流程下沉 DocumentNumberConfigService（date_format 空串归一回 '' 在服务内落实）
        $this->documentNumberConfigService->update($config, $request->validated());

        return $this->ok($this->payload($config->refresh()));
    }

    /** 规则预览：按提交的临时规则生成一条示例单号（不落库、不占号），供编辑弹窗核对 */
    public function preview(PreviewDocumentNumberConfigRequest $request)
    {
        $data = $request->validated();
        // date_format 允许空字符串（无日期段，如商品编码全局自增）：ConvertEmptyStringsToNull 会转 null，归一化回 '' 复用 update 方式
