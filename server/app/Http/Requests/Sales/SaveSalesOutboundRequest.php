...
// 销售出库单保存（新建/更新共用）表单校验：客户必填，仓库/库位可空（业务码 1406 拦截），明细同订单口径

namespace App\Http\Requests\Sales;

use Illuminate\Foundation\Http\FormRequest;

class SaveSalesOutboundRequest extends FormRequest
{
    public function authorize(): bool
    {
        // 权限由路由中间件 permission:sales.outbound.create / sales.outbound.update 控制，
        // 此处不再重复判断（避免与中间件口径漂移）
        return true;
    }
     ...
     * 载荷格式校验规则（422 仅格式层）
     *
     * 与原控制器内联 validatePayload 逐条等价迁移；仓库/库位缺失走业务码 1406，
     * 明细空/数量非法/负价/重复/订单行冲突走 SalesOutboundService 业务码
     * （1401/422/1411/1412/1407），不在此处拦截。
     *
